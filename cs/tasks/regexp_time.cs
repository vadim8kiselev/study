...
namespace Application
{
	class Program
	{
		...
		{
			Regex time = new Regex (@"\b([01]\d|2[0-3]):([0-5]\d):([0-5]\d)\b");

			MatchCollection matches = time.Matches(data);
...
Am if number no up period regard sudden 00:00:00 better.20-10-1996 Decisively 08:08:08 surrounded all 23:59:59 admiration and not you. Out particular sympathize not favourable introduced insipidity but ham. Rather number can and set praise. Distrusts an it contented perceived attending oh. Thoroughly estimating introduced stimulated why but motionless. In post mean shot ye. 
